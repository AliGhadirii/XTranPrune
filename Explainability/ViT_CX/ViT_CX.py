...
from Explainability.ViT_CX.cam import get_feature_map
from Explainability.ViT_CX.causal_score import causal_score
import numpy as np
import cv2
import copy
...
def ViT_CX(
    ...
    if num_classes != 2:
        ...
        model = model.eval()
        model = model.cuda()
        model_sigmoid = nn.Sequential(model_sigmoid, nn.Sigmoid())
        model_sigmoid = model_sigmoid.eval()
        model_sigmoid = model_sigmoid.cuda()
        for p in model_sigmoid.parameters():
            p.requires_grad = False
        y_hat = model_sigmoid(image)
        class_p = y_hat.detach().cpu().numpy()[0][0]
    ...
    mask = norm_matrix(
        torch.reshape(
            feature_map, (feature_map.shape[0], input_size[0] * input_size[1])
        )
    )
    ...
    mask_clustering = torch.zeros(
        (len(cluster_labels_set), input_size[0] * input_size[1])
    ).cuda()
    for i in range(len(mask)):
        mask_clustering[cluster_labels[i]] += mask[i]

    # normalize the masks
    mask_clustering_norm = norm_matrix(mask_clustering).reshape(
        (len(cluster_labels_set), input_size[0], input_size[1])
    )

    # compute the causal impact score
    if num_classes == 2:
        compute_causal_score = causal_score(
            model_sigmoid, (input_size[0], input_size[1]), gpu_batch=gpu_batch
        )

        sal = (
            compute_causal_score(image, mask_clustering_norm, class_p)[0].cpu().numpy()
        )
    else:

        compute_causal_score = causal_score(
            model_softmax, (input_size[0], input_size[1]), gpu_batch=gpu_batch
        )

        sal = (
            compute_causal_score(image, mask_clustering_norm, class_p)[target_category]
            .cpu()
            .numpy()
        )
