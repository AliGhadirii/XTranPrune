...
import torch
import ttach as tta
from Explainability.ViT_CX.cam.activations_and_gradients import ActivationsAndGradients
...
class BaseCAM:
    ...
    def compute_cam_per_layer(self, input_tensor, target_category, eigen_smooth):
        activations_list = [
            a.cpu().data.numpy() for a in self.featuremap_and_grads.featuremaps
        ]
        grads_list = [g.cpu().data.numpy() for g in self.featuremap_and_grads.gradients]
